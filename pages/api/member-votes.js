...
export default async function handler(req, res) {
  ...
  const voterUrl = \`https://www.govtrack.us/api/v2/vote_voter?person__bioguideid=\${bioguideId}&limit=600\`;
      ...
      if (relatedBill && relatedBill.id) {
        ...
        if (subjects.some(s => s.includes(subject.toLowerCase()))) {
          billVotes.push({
            ...
            congressUrl: \`https://www.congress.gov/bill/\${billData.congress}th-congress/\${billData.bill_type}/\${billData.number}\`,
            vote: votePosition
          });
        }
      }
